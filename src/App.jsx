...
function rotatePlayers(items, offset) {
...
function makeRound({ players, category, word, impostorCount, settings, impostors, startOffset = 0, passOrder }) {
  const order = passOrder || (settings.randomisePassOrder ? shuffle(players) : rotatePlayers(players, startOffset));
  return {
    settings,
    passOrder: order,
    category,
...
function App() {
  ...
  const [roundStartOffset, setRoundStartOffset] = useState(0);
  ...
  function startNewRound() {
    ...
    setRound(makeRound({ players, category: selected.category, word: selected.word, impostorCount, settings, startOffset }));
    setRoundStartOffset((current) => (players.length ? (current + 1) % players.length : 0));
  ...
  function removePlayer(name) {
    ...
    setRoundStartOffset((current) => nextPlayers.length ? current % nextPlayers.length : 0);
  }
...
function HomeScreen({ homeTab, setHomeTab, setupMode, setSetupMode, players, newPlayer, setNewPlayer, addPlayer, removePlayer, movePlayer, category, setCategory, categoryNames, customSetNames, settings, patchSettings, applyPreset, impostorCount, setImpostorCount, canStart, startRound, scores, resetScores, usedWordCount, totalWords, selectedWordCount, resetUsedWords, customSetName, setCustomSetName, customSetBase, setCustomSetBase, customSetWords, setCustomSetWords, saveCustomSet, deleteCustomSet }) {
  ...
  return <div className="screen-stack home-tab-layout"><section className="hero-card setup-summary compact-setup-summary"><img className="hero-logo" src={MOB_LOGO_SRC} alt="A$AP MOB Impostor crest" /><p className="eyebrow">Pre-game setup</p><h2>{homeTab === 'play' ? 'Choose mode and session.' : homeTab === 'players' ? 'Set the MOB lineup.' : homeTab === 'rules' ? 'Tune the rules.' : 'Manage the word library.'}</h2><div className="summary-grid"><div><span>Mode</span><strong>{setupMode === 'online' ? 'Online' : 'Local'}</strong></div><div><span>Players</span><strong>{players.length}</strong></div><div><span>Set</span><strong>{category}</strong></div><div><span>Words</span><strong>{selectedWordCount}</strong></div></div></section>
